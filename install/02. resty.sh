...
extract() {
  if ! test -d /src/${1} ; then
    ...
    if test -d ${my_dir}/resty/patches/${1} ; then
      pushd /src/$1
      for p in ${my_dir}/resty/patches/${1}/*.patch ; do
        patch -p1 < ${p}
      done
      popd
    fi
...
printf "Downloading Stream Lua Module ${STREAM_LUA_NGINX_MODULE_VERSION}\n" | tee -a ${DOWNLOAD_LOGFILE}
download \
  "https://github.com/openresty/stream-lua-nginx-module/archive/${STREAM_LUA_NGINX_MODULE_VERSION}.tar.gz" \
  "stream-lua-nginx-module-${STREAM_LUA_NGINX_MODULE_VERSION}.tar.gz" 2>&1 | tee -a ${DOWNLOAD_LOGFILE}
...
printf "Extracting Stream Lua Module...\n"
extract "stream-lua-nginx-module-${STREAM_LUA_NGINX_MODULE_VERSION}" ".tar.gz"
...
export DISTRO
export RELEASE
${my_dir}/resty/tools/lddsafe -nq "$PREFIX"/nginx/sbin/nginx | ${my_dir}/resty/tools/find-deps | while read dep; do
  ${my_dir}/resty/tools/mark-explicit "${dep}"
done
